...
const NewsList = () => {
  ...
  return (
       ...
       <div className={styles.wrapper__news}>
        <h1>Новости</h1>
        {liElements.map((item) => (
          <div style={{ backgroundImage: `url(${item.image})`, backgroundRepeat:'no-repeat', width: '80%', height: '300px'}} className={styles.main}>
            <h5 className={styles.header__news}>{item.title}</h5>
            <div className={styles.desc__news}>
            <Link className={styles.link} to={item.path}>Читать</Link>
